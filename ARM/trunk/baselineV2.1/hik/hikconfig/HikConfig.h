...
#define     MAX_BUSDETECTOR_NUM                 128      //公交车检测器最大数量
#define     MAX_SCARDETECTOR_NUM                128      //特勤车检测器最大数量
...
typedef struct STRU_N_BusDetector
{
	UInt8 byBusDetectorNumber;		//公交车检测器号
	UInt8 byBusDetectorCallChannel;	//公交车检测器对应的请求相位,配置为通道
	UInt32 byBusTagID;	//公交车检测的车辆标签ID(177路)
	UInt32 byRFIDReaderId; //RFID阅读器ID
	UInt32 passTime;   //公交车检测器到达停车线的间隔时间,即公交车通过的放行时间
	UInt8 reserve[16];//保留
}STRU_N_BusDetector;

typedef struct STRU_N_SpecialCarDetector
{
	UInt8 bySCarDetectorNumber;		//特勤车检测器号
	UInt8 bySCarDetectorCallChannel;	//特勤车检测器对应的请求相位,配置为通道
	UInt32 bySCarTagID;	//特勤车检测的车辆标签ID
	UInt32 byRFIDReaderId; //RFID阅读器ID
	UInt32 passTime;   //特勤车通过的放行时间
	UInt8 specialLevel; //特勤级别默认2级，执行跳转；一级执行通道锁定
	UInt8 reserve[15];//保留
}STRU_N_SpecialCarDetector;
...
typedef struct {
	...
	UINT16 pedKeyDelayTime;    //有请求后开始放行的延迟时间，单位为秒，为0表示立即放行
    UINT16 maxWaitTime;        //强制放行的时间间隔，单位为分，为0表示无请求始终不放行 
	UINT16 pedKeyPassGap;      //两次放行之间的最小时间间隔，单位为秒，为0表示可以立即响应
	//UINT16 singleSpotCheckTime; //单点优化检测时间，表示绿灯最后几秒开始
	UINT8 reserved[7];
