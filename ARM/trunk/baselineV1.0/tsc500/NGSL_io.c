...
extern UINT8 gFollowPhaseGreenBlinkFlag[NUM_PHASE];
...
void HardwareWatchdogInit()
{
	...
	if(is_WatchdogEnabled == 1)
	{
		...
		{
			ERR("Open watchdog error,retry once!!!\n");
			HardwareWatchdoghd = open("/dev/watchdog", O_WRONLY);
			//return;
		}
		//激活硬件看门狗
...
{
	UINT8 ret=0;
	if(pNewChannnelLockedParams->uChannelLockFlag == 0 || gStructBinfileCustom.cChannelLockFlag == 1)//原通道锁定优先
		return 0;
...
uint16 PedestrianCheckInput(void)
{
	...
	ret = WirelessKeyCheck();
	if(ret!=0 && WIRELESS_CTRL_ON == gStructBinfileConfigPara.stWirelessController.iSwitch)
		KeyControlDeal(ret);
...
*
***********************************************************************************/
extern int KeyDefaultOperation(int result);
int ProcessKey(void)
{
	int result = 0;
	char tmpstr[256] = "";
	...
	return KeyDefaultOperation(result);
...
{	
	...
	system("sleep 5 && /root/RTSC_sadp >& /dev/null &");//将打印重定向到NULL，不显示
