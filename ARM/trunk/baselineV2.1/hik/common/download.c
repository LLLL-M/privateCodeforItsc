...
void DownloadConfig(int type, void *arg)
{
	switch (type)
	{
		...
		case 0xae: StorePedestrianDetector(arg); SET_BIT(iIsSaveHikconfig, E_TABLE_NAME_PEDESTRIAN); break;
		...
		{	
			switch(i)
			{
				...
				case E_TABLE_NAME_VEHICLE:
					...
					break;
				case E_TABLE_NAME_PEDESTRIAN:
					sqlite3_clear_table(pdatabase, TABLE_NAME_PEDESTRIAN);
					sqlite3_insert_pedestrian(pdatabase, pconfig->AscPedestrianDetectorTable);
					break;
...
void LoadLocalConfigFile(void *config)
{
...
#endif
	//sprintf(db_bak_file, "%s.bak", DATABASE_HIKCONFIG_BAK);
	//INFO("%s", db_bak_file);
	...
	{
		if (IS_FILE_EXIST(DATABASE_HIKCONFIG_BAK) == 0)
        {
