...
static void Init485Serial()
{
    ...
    int fd_file= open(DEVICE_NAME_CPLD,O_RDONLY);
    ioctl(fd_file,TTYS4_SEND_ENABLE,&arg2);
}
/*****************************************************************************
 函 数 名  : Send485Data
...
void SetCountDownValue(PHASE_COUNTING_DOWN_FEEDBACK_PARAMS *pCountDownParamsSend)
{
    ...
    for(i = 0; i < MAX_NUM_PHASE; i++)
    {
        ...
        g_CountDownParams[MOTOR-1][i].cColor = pCountDownParamsSend->stVehPhaseCountingDown[i][0];
        g_CountDownParams[MOTOR-1][i].cTime = pCountDownParamsSend->stVehPhaseCountingDown[i][1];

        //行人倒计时
        g_CountDownParams[PEDESTRIAN-1][i].cControllerID = i+1;
        g_CountDownParams[PEDESTRIAN-1][i].cColor = pCountDownParamsSend->stPedPhaseCountingDown[i][0];
        g_CountDownParams[PEDESTRIAN-1][i].cTime = pCountDownParamsSend->stPedPhaseCountingDown[i][1];

        //跟随相位倒计时
        g_CountDownParams[FOLLOW-1][i].cControllerID = i+1;
        g_CountDownParams[FOLLOW-1][i].cColor = pCountDownParamsSend->ucOverlap[i][0];
        g_CountDownParams[FOLLOW-1][i].cTime = pCountDownParamsSend->ucOverlap[i][1];
...
*****************************************************************************/
void CountDownInterface()
{
    switch(gStructBinfileCustom.sCountdownParams.iCountDownMode)
    {
        case SelfLearning:      break;
        case FullPulse:         break;
        case HalfPulse:         break;
        case NationStandard:    NationStandardCountDownRun();break;
        case LaiSiStandard:     LaiSiCountDownRun(); break;
        case HisenseStandard:   HisenseStandardCountDownRun();break;
        default:                break;
    }
}
