...
#endif
#ifndef NUM_SPECIAL_DATE
#define NUM_SPECIAL_DATE			40	//最大特殊日期个数
#endif
...
typedef struct
{
	...
	//UInt32 ret:16;			//返回值，0:成功，1:有人正在下载，2:有人正在上载
...
typedef struct
{
	UInt8 week[7];//对应星期一到星期天的时段号
	YK_SpecialDate  specialDate[NUM_SPECIAL_DATE];//40 个特殊日设置
} YK_PlanScheduleTable;
...
typedef YK_TimeIntervalItem YK_TimeIntervalTable[NUM_TIME_INTERVAL_ID];
...
typedef YK_ChannelItem YK_ChannelTable[NUM_CHANNEL];
...
typedef struct
{
	...
	UInt8 signalMachineType:4;		//信号机类型选项,有三种信号机TSC500、TSC300-44、TSC300-22,对应的值分别为0、1、2
...
typedef struct
{
	UInt8	lockFlag;	//0:解锁，1:锁定
	UInt8   ucChannelStatus[NUM_CHANNEL];		//32个通道状态，数据如枚举LightStatus所示
...
typedef struct
{
	int startLine;	//其实行号，正数从头开始读取，负数从尾部即倒着读取
	int linenum;	//读取的行数，必须为正数，最大只能读取127个
} YK_FaultLog;
...
typedef struct
{
	UInt32 networkType;	//0x15f为网口0,0x15d为网口1,0x161为无线wlan
	UInt32 ip;
	UInt32 netmask;
	UInt32 gateway;
} YK_NetworkParam;
...
typedef struct
{
	YK_PlanScheduleTable scheduleTable;
	YK_TimeIntervalTable timeIntervalTable[NUM_TIME_INTERVAL];
	YK_ChannelTable channelTable;
