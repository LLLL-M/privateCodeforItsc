...
*****************************************************************************/
static int checkPhaseTrunInRing(SignalControllerPara *pSignalControlpara,unsigned short phaseTurn,int nPhaseTableNo)
{
...
//判断通道表是否合法
static int IsChannelLegal(SignalControllerPara *pSignalControlpara,unsigned short *nPhaseArray,unsigned short *nFollowPhaseArray,int nChannelNo)
{
    int i = 0;
    int sTemp = 0;
...
static int IsSchemeLegal(SignalControllerPara *pSignalControlpara,int nSchemeId,int nPhaseTableNo) 
{
    ...
    if((nSchemeId >= 251) && (nSchemeId <= 255))//特殊方案号
    {
        return 0;
    }
...
static int IsActionLegal(SignalControllerPara *pSignalControlpara,int nActionId)
{
    ...
    if(IsSchemeLegal(pSignalControlpara,nSchemeId,nPhaseTableNo) != 0)//判断方案表是否合法
    {
        return 1;
    }
    //判断相位表是否合法,这个在方案表中判断
    ...
    //判断跟随相位表
    if(IsFollowPhaseLegal(pSignalControlpara,nPhaseTableNo) != 0)
    {
        return 1;
    }
    ...
    if(IsChannelLegal(pSignalControlpara,nPhaseArray,nFollowPhaseArray,nChannelTableNo) != 0)
    {
        return 1;
    }
    return  0;
...
Boolean LoadDataFromCfg(SignalControllerPara *pSignalControlpara, const char *path)
{
    ...
    {
        ReadPhaseItem(pSignalControlpara->stPhase[i],pSignalControlpara->AscSignalTransTable[i],NUM_PHASE,i+1);
    }

    //加载通道参数 
    for(i = 0 ; i < MAX_CHANNEL_TABLE_COUNT; i++)
    {
        ReadChannelItem(pSignalControlpara->stChannel[i], NUM_CHANNEL,i+1);
    }
    ...
    {
        ReadFollowPhaseItem(pSignalControlpara->stFollowPhase[i],NUM_FOLLOW_PHASE,i+1);
    }    
