	...
	for(i = 0 ; i < NUM_TIME_INTERVAL; i++)
    {
        for(j = 0 ; j < NUM_TIME_INTERVAL_ID ; j++)
        {
            ...
            if(gSignalControlpara->stTimeInterval[i][j].nActionID != 0)
                INFO("%s  stTimeInterval  nTimeIntervalID  %d  , nTimeID  %d , cStartTimeHour  %d , cStartTimeMinute  %d , nActionID  %d   \n",__func__,
...
static void loadPlanScheduleFromIni()	//装载调度表
{
    ...
    for(i = 0 ; i < NUM_SCHEDULE; i++)
    {
        ...
        if(gSignalControlpara->stPlanSchedule[i].nTimeIntervalID != 0)
            INFO("%s  stPlanSchedule  nScheduleID  %d  , nTimeIntervalID  %d, month = %d, week = %d, day = %u\n",__func__,
...
void storeTimeIntervalToIni(void *arg)
{
	...
	for (i = 0; i < SEND_TIMEINTERVAL_NUM; i++) {
		...
		INFO("[stTimeInterval]  nTimeIntervalID: %d, nTimeID: %d, cStartTimeHour: %d, cStartTimeMinute: %d, nActionID: %d\n",
...
void storePlanScheduleToIni(void *arg)
{
	...
	for (i = 0; i < SEND_PLANSCHEDULE_NUM; i++) {
		...
		INFO("[stPlanSchedule] nScheduleID: %d, nTimeIntervalID: %d, month: %d, week: %d, day: %u\n",
				item[i].nScheduleID, 
				item[i].nTimeIntervalID,
				item[i].month,
				item[i].week,
...
//向本机161端口发送
	...
	if(len < 0)
	{
		ERR("Send inductive control signal failed!!!\n");
	}
	close(socketFd);
...
static void udp_send_change_ctrl()
{
	...
	if(len < 0)
	{
		ERR("Send system control signal failed!!!\n");
	}
	close(socketFd);
...
//#define DEBUG
    ...
    if (flag == INIT) {	//初始化
		if ((gSignalControlpara = (SignalControllerPara *)calloc(1, sizeof(SignalControllerPara))) == NULL) {
			ERR("memory is not enough to store the backup config information\n");
			return;
		}
		flag = NOUSED;
