...
extern void *BusDetectorCom(void *arg);
...
UINT32 gCurrentScheme = 0;
UINT32 gCurrentAction = 0;
UINT8  gInductiveDemotion = 0;
UINT8  gCurControlType = 0;
	...
	while (1)
	{
		...
		if (ifr.ifr_flags & IFF_RUNNING)
		{
			...
			{	//网络畅通后恢复到系统控制
				...
				ClearVehDetectorState();//clear veh detector's error state
			}
			/*else if (gCurrentAction == INDUCTIVE_ACTIONID)//inductive coordinate or inductive
			{				
				if (GetVehDetectorState() > 0 && gInductiveDemotion == 0)//get vehicle detector's erroe state ,bigger than 0 means some error occur
				{
					//vehDetectorErr = 1;
					//log_debug("vehicle detector occur error");
					if (IsExcuteDemotion(&demotionSchemeId))
					{
						if (((gCurrentScheme - 2) % 3) == 0)
							demotionSchemeId = gCurrentScheme - 1;
						if (gCurControlType == AUTO_CONTROL)
							ctrl_mode = AUTO_CONTROL;
						else if (gCurControlType == TOOL_CONTROL)
							ctrl_mode = TOOL_CONTROL;
						ItsCtlNonblock(TOOL_CONTROL, demotionSchemeId, demotionSchemeId);
						gInductiveDemotion = 1;
						log_debug("vehicle detector occur error, demotionSchemeId = %d!", demotionSchemeId);
					}
				}
				
			}
			if (GetVehDetectorState() == 0)
			{
				if (gInductiveDemotion == 1)//recover from error state
				{
					log_debug("vehicle detector recover normal");
					//vehDetectorErr = 0;
					//if ((gCurrentScheme - 1) % 3 == 0)
						//demotionSchemeId = gCurrentScheme + 1;
					//ItsCtlNonblock(AUTO_CONTROL, INDUCTIVE_SCHEMEID, demotionSchemeId);
					if (ctrl_mode == AUTO_CONTROL)
						ItsCtl(TOOL_CONTROL, 0, 0);
					else if (ctrl_mode == TOOL_CONTROL)
					{
						if ((gCurrentScheme - 1) % 3 == 0)
							demotionSchemeId = gCurrentScheme + 1;
						ItsCtl(TOOL_CONTROL, INDUCTIVE_SCHEMEID, demotionSchemeId);
					}
					gInductiveDemotion = 0;
				}
			}
			*/
		}
...
static void CheckDatabaseBak()
{
	char cmd[144] = {0};
	
	if (IS_FILE_EXIST(DATABASE_HIKCONFIG) == 0)
		if (IS_FILE_EXIST(DATABASE_HIKCONFIG_BAK) == 1)
		{
			sprintf(cmd, "cp -f %s %s", DATABASE_HIKCONFIG_BAK, DATABASE_HIKCONFIG);
			system(cmd);
		}
	memset(cmd, 0, 144);
	if (IS_FILE_EXIST(DATABASE_EXTCONFIG) == 0)
		if (IS_FILE_EXIST(DATABASE_EXTCONFIG_BAK) == 1)
		{
			sprintf(cmd, "cp -f %s %s", DATABASE_EXTCONFIG_BAK, DATABASE_EXTCONFIG);
			system(cmd);
		}
}
...
void InitCommon()
{
	struct threadInfo threads[] = {
		...
		{0, BusDetectorCom, "rfid bus detector", 0},
#endif
