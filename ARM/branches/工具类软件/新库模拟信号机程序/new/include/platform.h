...
typedef struct STRU_Extra_Param_Phase_Counting_Down_Feedback
{
	 ...
	 //以下内容是Jicky擅自添加的，倒计时上载时暂时不包含以下数据
	 unsigned char	  ucChannelRealStatus[32];	//实际的通道状态，用来上载通道状态时使用，便于统计
	 unsigned char	  ucChannelCountdown[32];	//实际的通道倒计时，用来上载通道倒计时状态时使用，便于统计，给触摸屏使用
