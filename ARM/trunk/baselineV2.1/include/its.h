...
#define PEDESTRIAN_REQ_ACTIONID 110
#define BUS_PRIORITY_ACTIONID   109
#define SINGLE_OPTIMIZE_ACTIONID 108
...
#define PEDESTRIAN_REQ_SCHEMEID 246   //行人请求过街方案号
#define BUS_PRIORITY_SCHEMEID   245         //公交优先方案号
#define SINGLE_OPTIMIZE_SCHEMEID 244     //单点优化方案号
...
typedef enum
{
	...
	RING_CONCURRENCY_ILLEGAL=0x60,                  //环并发配置不合理
	CHANNEL_LOCK_COLLISION=0x61,                    //通道锁定冲突错误
	VEHICLE_DETECTOR_NORESPONSE=0x62,               //车检器最大无响应错误
	VEHICLE_DETECTOR_NORESPONSE_CLR=0x63,           //车检器最大无响应错误清除
	VEHICLE_DETECTOR_PASSLIMIT=0x64,                //车检器的单位过车数超出限制错误
	VEHICLE_DETECTOR_PASSLIMIT_CLR=0x65,            //车检器的单位过车数超出限制错误清除
	VEHICLE_DETECTOR_MAXPRESENCE=0x66,              //车检器最大持续额时间错误
	VEHICLE_DETECTOR_MAXPRESENCE_CLR=0x67,          //车检器最大持续额时间错误清除
...
typedef enum 
{ //特勤车辆控制的级别
	SPECIALCAR_LEVEL1 = 1,//特勤级别1 通道锁定
	SPECIALCAR_LEVEL2 = 2,//特勤级别2 跳相位
}SpecialCarControlLevel;
...
typedef struct
{
	...
	UINT8  vechileAutoReq;      //车辆自动请求标志，1 auto，0 detector
	UInt16 pedestrianDetectorBits;	//相位对应的行人检器号
	UINT8 pedestrianAutoReq;        //相位的行人自动请求1 auto, 0 detector
	UINT32 busDetectorBits[4];      //相位对应的公交车检测器号,128bit
	UINT32 scarDetectorBits[4];     //特勤车辆对应的检测器号，128bit
	UINT32 busPrioPassTime;         //公交检测到停车线延迟时间
	UINT32 scarPassTime;            //特勤车辆放行时间
	UInt8 unitExtendGreen;		//单位延长绿
	INT8 maxExtendGreen;		//最大可以延长的绿灯时间
	INT8 maxExtendGreen2;      //自适应控制的最大可以延长的绿灯时间
	INT8 minShortGreen;        //最大可缩短绿灯时间
...
typedef struct SpecialCarControl
{	
UINT8 SpecialCarControlLevel; //特勤车辆控制级别
INT8 SpecialCarControlTime;  //特勤车辆控制时间
UINT8 SpecialCarStepStage;    //特勤车辆跳转阶段
UINT8 SpecialCarPhase;        //特勤车辆对应相位
UINT8 SpecialCarChanLockStatus[32];  //特勤车辆通道锁定状态
}SpecialCarControl;

typedef struct stepInductiveInfo
{
	UINT8 stepStageNum;//next step stagenum, 0, current phase don't need to step; big than 0, step to stagenum; 
	UINT8 stepNow; // 2, current channel green lefttime between check time and last one second, 1 green lefttime == 1, 0 green letftime > check time
}StepInductiveInfo;
...
WEAKATTR extern int ItsCalcuInductiveStepStage(CalInfo *pcalinfo, UINT8 stagenum);
WEAKATTR extern int ItsGetOneSecVehPass(UINT8 inwindow, UINT64* vehdata_1s);
WEAKATTR extern void ItsClearVehDataHaveCars(CalInfo *pcalinfo, UINT8 current_phase);
WEAKATTR extern UINT8 ItsReadPedestrianDetectorData(UINT8 readNowFlag);
WEAKATTR extern UINT8 ItsClearPedestrianDetectorData(LineQueueData* data, UINT8 phaseId, UINT8 pedkey);
WEAKATTR extern int ItsClearStepPhaseTimeGap(CalInfo *pcalinfo, UINT8 phaseid);
WEAKATTR extern UINT16 ItsGetPedKeyDelayTime();
WEAKATTR extern UINT8 ItsResetVehicleInductiveData();
WEAKATTR extern UINT32 ItsGetBusPrioData(UINT32* busData);
WEAKATTR extern UINT32 ItsClearBusPrioData(LineQueueData *data, UINT8 phaseId);
WEAKATTR extern UINT8 ItsResetNextCycleExtendTime();
WEAKATTR extern UINT8 ItsGetSpecialCarControlData(CalInfo *pcalinfo, SpecialCarControl* scarControl);
WEAKATTR extern UINT8 ItsClearSpecialCarDataBits(CalInfo *pcalinfo, UINT8 phaseId);
WEAKATTR extern void ItsAddSpecialCarChanLock(SpecialCarControl* scarControl);
WEAKATTR extern void ItsDelSpecialCarChanLock();
WEAKATTR extern UINT8 ItsSetReadPedestrianReqTime(void);
WEAKATTR extern void ItsGetCurControlType(UINT8 controltype);
