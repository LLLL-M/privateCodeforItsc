...
//	char seqNum1[12], seqNum2[12], seqNum3[12],seqNum4[12];
	...
	if (reqmethod == METHOD_GET)
	{
		...
		result = getProgramTableInfo(&gProgramTable, account*3-2);
	}
	else if(reqmethod == METHOD_POST)
	{
		//如果是PUT命令，则从客户端发送过来的xml进行解析并保存到配置文件中，并增加设置是否成功的结果一起返回
		getXMLValue(query,"ProgramTable","ProgramTableNo",tmpStr);
		gProgramTable.nSchemeID = atoi(tmpStr)*3-2;
...
void timeBasedActionTable(webs_t wp, char_t *path, char_t *query)
{
	...
	{		//如果是PUT命令，则从客户端发送过来的xml进行解析并保存到配置文件中，并增加设置是否成功的结果一起返回
		...
		gTimeBasedActionTable.ProgramNo = atoi(tmpStr)*3-2;
		...
		gTimeBasedActionTable.ProgramNo = (gTimeBasedActionTable.ProgramNo +2 )/3;
	}
...
//把以,为分隔符的字符串转换成数组
static int Strlist2IntArray(char *str,int *array,int len)
{
    int i = 0;
    char *p = NULL;
...
//判断某个值是否在数组中
static int IsItemInArray(int *array,int len,int val)
{
    int i = 0;
...
//根据动态树，更新全局参数  blockSize 对应于二维数组的行数，num对应于二位数组的列数
void UpdateSignalParams(int size,int blockSize,int num,char *str,UINT8 *val)
{
    int i = 0;
    int j = 0;
    int nArray[128];
    ...
    nArraySize = Strlist2IntArray(str, nArray,128);
    
    for(i = 0;i < size; i++)
    {
        if(size == NUM_SCHEME)//方案表需要做转换
            temp = (i+1+2)/3;
        else
            temp = i+1;
        if(IsItemInArray(nArray,nArraySize,temp) == 0)//如果不在数组中，就把val清零
        {
            for(j = 0; j < num; j++)
            {
                if(num == 1)
...
void TreeDynamicParameter(webs_t wp, char_t *path, char_t *query)
{
	...
	{		//如果是PUT命令，则从客户端发送过来的xml进行解析并保存到配置文件中，并增加设置是否成功的结果一起返回
        ...
        UpdateSignalParams(NUM_PHASE,sizeof(PhaseItem),1,tmpStr,&gSignalControlpara->stPhase[0].wPhaseOptions);

        //channel arry
        getXMLValue(query,"TreeDynamicParameter","channelArray",tmpStr);
        UpdateSignalParams(NUM_CHANNEL,sizeof(ChannelItem),1,tmpStr,&gSignalControlpara->stChannel[0].nControllerID);

        //方案表
        getXMLValue(query,"TreeDynamicParameter","programArray",tmpStr);
        UpdateSignalParams(NUM_SCHEME,sizeof(SchemeItem),1,tmpStr,&gSignalControlpara->stScheme[0].nCycleTime);

        //绿信比表
        getXMLValue(query,"TreeDynamicParameter","splitArray",tmpStr);
        UpdateSignalParams(NUM_GREEN_SIGNAL_RATION,sizeof(GreenSignalRationItem),NUM_PHASE,tmpStr,&gSignalControlpara->stGreenSignalRation[0][0].nGreenSignalRationTime);

        //相序表
        getXMLValue(query,"TreeDynamicParameter","phaseTurnArray",tmpStr);
        UpdateSignalParams(NUM_PHASE_TURN,sizeof(PhaseTurnItem),NUM_RING_COUNT,tmpStr,&gSignalControlpara->stPhaseTurn[0][0].nTurnArray[0]);

        //动作表
        getXMLValue(query,"TreeDynamicParameter","ActionArray",tmpStr);
        UpdateSignalParams(NUM_ACTION,sizeof(ActionItem),1,tmpStr,&gSignalControlpara->stAction[0].nSchemeID);

        //时段表
        getXMLValue(query,"TreeDynamicParameter","TimeIntervalArray",tmpStr);
        log_debug("TimeIntervalArray  %s\n",tmpStr);
        UpdateSignalParams(NUM_TIME_INTERVAL,sizeof(TimeIntervalItem),NUM_TIME_INTERVAL_ID,tmpStr,&gSignalControlpara->stTimeInterval[0][0].nActionID);

        //跟随相位表
        getXMLValue(query,"TreeDynamicParameter","FollowPhaseArray",tmpStr);
        UpdateSignalParams(NUM_FOLLOW_PHASE,sizeof(FollowPhaseItem),1,tmpStr,&gSignalControlpara->stFollowPhase[0].nArrayMotherPhase[0]);

        //调度表
        getXMLValue(query,"TreeDynamicParameter","ScheduleArray",tmpStr);
        UpdateSignalParams(NUM_SCHEDULE,sizeof(PlanScheduleItem),1,tmpStr,&gSignalControlpara->stPlanSchedule[0].nTimeIntervalID);

        // 车辆检测器
        getXMLValue(query,"TreeDynamicParameter","VehicleDetectorArray",tmpStr);
        UpdateSignalParams(MAX_VEHICLEDETECTOR_COUNT,sizeof(struct STRU_N_VehicleDetector),1,tmpStr,&gSignalControlpara->AscVehicleDetectorTable[0].byVehicleDetectorCallPhase);

        //行人检测器
        getXMLValue(query,"TreeDynamicParameter","PedestrianDetectorArray",tmpStr);
        UpdateSignalParams(MAX_PEDESTRIANDETECTOR_COUNT,sizeof(struct STRU_N_PedestrianDetector),1,tmpStr,&gSignalControlpara->AscPedestrianDetectorTable[0].byPedestrianDetectorCallPhase);
    ...
    for(i = 0 ; i < NUM_GREEN_SIGNAL_RATION ; i++)
    {
        for(j = 0 ; j <  NUM_PHASE; j++)
        {
            if(gSignalControlpara->stGreenSignalRation[i][j].nGreenSignalRationTime != 0)
            {
    ...
    {
        if((gSignalControlpara->stScheme[i].nGreenSignalRatioID != 0) && 
			(gSignalControlpara->stScheme[i].nPhaseTurnID != 0)&&
			(gSignalControlpara->stScheme[i].nCycleTime != 0))
        {
            gTreeDynamicPara.addProgram++;
        //    sprintf(tmpStr+strlen(tmpStr),"%d,",gSignalControlpara->stScheme[i].nSchemeID);
            sprintf(tmpStr+strlen(tmpStr),"%d,",gTreeDynamicPara.addProgram);
		}
    }
    tmpStr[strlen(tmpStr) - 1] = '\0';
    ...
    for(i = 0 ; i < NUM_ACTION ; i++)
    {
        if((gSignalControlpara->stAction[i].nActionID != 0) 
            ...
            && (gSignalControlpara->stAction[i].nSchemeID != 0)) 
        {
