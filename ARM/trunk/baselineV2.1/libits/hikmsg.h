...
#define AHEAD_OF_TIME	2	//下一个周期提前6s开始计算
...
typedef enum 
{
	...
	MSG_CHANNEL_STATUS,		//通道控制模块发送给相位驱动模块的通道状态
...
typedef enum
{
	...
	SINGLE_ADAPT_MODE = 7,          //自适应控制
	//以下4个是国标新增的控制方式
	SINGLE_SPOT_OPTIMIZE = 8,		//单点优化
	PEDESTRIAN_REQ_MODE = 9,        //行人过街按键请求
	BUS_PRIORITY_MODE = 10,      //公交优先
...
#define CLEAR_FAULT_LOG	3
			...
			int startLine;
			int lineNum;
			UploadFaultLogFunc func;	//上载故障日志的函数指针
			char netArg[0];	//用来存放上载故障日志时的网络相关参数，大小不能超过(MSGSIZE - sizeof(struct faultLogMsg))
		} faultLog;
	} mdata;
