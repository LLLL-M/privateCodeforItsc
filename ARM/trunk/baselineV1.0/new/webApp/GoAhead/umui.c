...
static void     formChangePassword(webs_t wp, char_t *path, char_t *query);
...
void formDefineUserMgmt(void)
{
	...
	websFormDefine(T("ChangePassword"), formChangePassword);
...
static void formChangePassword(webs_t wp, char_t *path, char_t *query)
{
	char_t	userid[16], pass1[16], pass2[16], *password;
	int	nCheck;

	a_assert(wp);

	getXMLValue(query,"passwordChange","user",userid);
	getXMLValue(query,"passwordChange","oldpassword",pass1);
	getXMLValue(query,"passwordChange","newpassword",pass2);
#if 0
	userid = websGetVar(wp, T("user"), T("")); 
	pass1 = websGetVar(wp, T("oldpassword"), T("")); 
	pass2 = websGetVar(wp, T("newpassword"), T("")); 
#endif
	printf("%s: userid=%s, pass1=%s, pass2=%s\n", __func__, userid, pass1, pass2);
	websHeader(wp);
	websMsgStart(wp);

    if (!umUserExists(userid))
	{
		printf("User %s isn't exist!\n", userid);
        websWrite(wp, T("User %s isn't exist!"), userid);
	}
    else
    {
        password = umGetUserPassword(userid);
        if (gstrcmp(password, pass1) == 0)
        {
            if (umSetUserPassword(userid, pass2) == 0)
			{
				printf("change user %s password %s successful!\n", userid, pass2);
                websWrite(wp, T("change user %s password successful!"), userid);
			}
            else
			{
				printf("change user %s password %s fail!\n", userid, pass2);
                websWrite(wp, T("change user %s password fail!"), userid);
			}
        }
        else
		{
			printf("Password %s input is wrong, it shoule be %s!\n", pass1, password);
            websWrite(wp, T("Password %s input is wrong!"), pass1);
		}
    }
	websMsgEnd(wp);
	websFooter(wp);
