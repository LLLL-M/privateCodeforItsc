 ...
 */
	...
	printf("userid:%s, password:%s, url:%s, path:%s, cookie:%s\n", userid, password, url, path, wp->cookie);
#if 1 //change by Jicky
	if ((gstrcmp(path, T("/webs/doc/page/paramconfig.asp")) == 0)
		&& (!userid || !password) && !wp->cookie) {
		puts(">>>>>>>>>>> websRedirect <<<<<<<<<<<<<<<<<");
