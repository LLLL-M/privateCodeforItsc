 ...
 * 宏定义                                       *
 *----------------------------------------------*/
#define HIKCONFIG_LIB_VERSION	"1.0.0"

#define CFG_NAME "/home/hikconfig.ini"
...
typedef enum
{
	ERROR_NULL_POINTER = 1,						//pSignalControlpara指针为空
	ERROR_SUBSCRIPT_PHASE,				 		//相位数组下标值+1与实际相位值不相同。nPhaseArray[i] != (i+1)
	ERROR_SUBSCRIPT_PHASE_TURN,				    //相序表的数组下标值+1与实际相序表号不相同。
	ERROR_SUSCRIPT_PHASE_TURN_CIRCLE,			//相序表的环号组的下标值+1与实际环号不相同。
	ERROR_SUBSCRIPT_GREEN_SIGNAL_RATION,		//绿信比表的数组下标值+1与实际绿信比表号不相同。
	ERROR_SUBSCRIPT_GREEN_SIGNAL_RATION_PHASE,	//绿信比表的相位数组下标值+1与实际相位号不相同。
	ERROR_SUBSCRIPT_CHANNEL,					//通道表的数组下标值+1与实际通道号不相同。
	ERROR_SUBSCRIPT_SCHEME,					//方案表的数组下标值+1与实际方案号不相同。
	ERROR_SUBSCRIPT_ACTION,					//动作表的数组下标值+1与实际动作号不相同。
	ERROR_SUBSCRIPT_TIMEINTERVAL,				//时段表的数组下标值+1与实际时段表号不相同。
	ERROR_SUBSCRIPT_TIMEINTERVAL_TIME,			//动作表的时段数组下标值+1与实际时段号不相同。
	ERROR_SUBSCRIPT_SCHEDULE,					//调度表的数组下标值+1与调度表号不相同。
	ERROR_SUBSCRIPT_FOLLOW_PHASE,				//跟随相位表的数组下标值+1与实际跟随相位号不相同。
	
	ERROR_SAME_CIRCLE_CONCURRENT_PHASE,		//并发相位表中相位号存在于并发相位数组中。
	ERROR_FATHER_NOT_EXIST_CONCURRENT_PHASE,	//并发相位的相位号不存在于其并发相位的并发相位数组中。
	ERROR_CHILD_NOT_CONTINUOUS_CONCURRENT_PHASE, //并发相位的子相位在相序表中不连续。
	ERROR_BARRIER_CONCURRENT_PHASE, 			//并发相位的屏障一侧绿信比时间不相等。
	ERROR_NOT_EQUAL_PHASE_TURN_CIRCLE,			//相序表中环号和相位表中该相位的环号不相同。
	ERROR_NOT_CORRECT_PHASE_TURN,				//相序表中相位的序号和并发相位不匹配
	ERROR_SPLIT_LOW_MOTO_GREEN_SIGNAL_RATION,	//绿信比表中周期长比机动车的绿闪、黄灯、全红之和小
	ERROR_SPLIT_LOW_PEDESTRIAN_GREEN_SIGNAL_RATION,//绿信比表中周期长比行人清空、行人放行时间之和小。
	ERROR_ILLEGAL_SCHME,						//方案表中相序表或绿信比表ID非法。
	ERROR_CIRCLE_TIME_SCHEME,					//方案表中周期长不等于单环绿信比之和
	
	ERROR_NOT_EXIST_CONCURRENT_PHASE_PHASE,	//并发相位表中的并发相位ID不存在。
	ERROR_NOT_EXIST_PHASE_TURN_PHASE,			//相序表中相位号不存在。
	ERROR_NOT_EXIST_SOURCE_CHANNEL,				//通道表中该通道的控制源ID不存在。
	ERROR_NOT_EXIST_SOURCE_FOLLOW_CHANNEL,		//通道表中该通道的跟随控制源ID不存在。
	ERROR_NOT_EXIST_SOURCE_OTHER_CHANNEL,		//通道表中该通道的其他控制源ID不存在。
	ERROR_NOT_EXIST_PHASE_TURN_SHCEME,			//方案表中相序表不存在。
	ERROR_NOT_EXIST_GREEN_SIGNAL_RATION_SCHEME,	//方案表中绿信比表不存在。
	ERROR_NOT_EXIST_SCHEME_ACTION,				//动作表中方案表不存在。
	ERROR_NOT_EXIST_ACTION_TIMEINTERVAL,		//时段表中动作表不存在。
	ERROR_NOT_EXIST_TIMEINTERVAL_SCHEDULE,		//调度表中时段表不存在。
	ERROR_NOT_EXIST_MOTHER_PHASE_FOLLOW_PHASE,	//跟随表中母相位不存在。
	ERROR_NOT_EXIST_CONCURRENT_PHASE_PHASE_2,	//相位表中有相位不存在并发相位。
	ERROR_NOT_EXIST_PHASE_TURN_PHASE_2,			//相位表中有相位不存在于相序表中。
	ERROR_NOT_EXIST_PHASE_GREEN_SIGNAL_RATION,	//相位表中有相位不存在与绿信比表中。
	
	ERROR_REPEATE_CONCURRENT_PHASE,			//某相位的并发相位数组有重复值。
	ERROR_REPEATE_PHASE_TURN,					//某相序表的相序数组有重复值。
	ERROR_REPEATE_FOLLOW_PHASE,				//跟随相位表的跟随相位数组有重复值。
	ERROR_REPEATE_SCHEDULE,						//调度表中有重复日期调度
	
	ERROR_NOT_CONFIG_INFORMATION,				//没有配置信息
}TSC_Para_Error_Num;
...
typedef struct {
    ...
    UInt8  nUnitExtendGreen;                 /*2.2.2.5单位绿灯延长时间(0-25.5 sec)，简称单位延长绿。(0..255)*/
    UInt8  nMaxGreen_1;                      /* 2.2.2.6最大绿灯时间1，简称最大绿1。(0..255)*/
    UInt8  nMaxGreen_2;                      /* 2.2.2.7最大绿灯时间2，简称最大绿2。(0..255)*/
    UInt8  nYellowTime;                      /*2.2.2.8相位黄灯时间*/
    UInt8  nAllRedTime;			            /*2.2.2.9 NTCIP称作相位红灯清空时间,也就是我们通常所说的全红时间*/
...
typedef struct {
    ...
    UInt8  cAuxiliary;                       /* 2.6.3.3感应控制时进行辅助功能选择。
                        				Bit 7: Reserved
                        				Bit 6: Reserved
                        				Bit 5: Reserved
                        				Bit 4: Reserved
                        				Bit 3:灰度控制（1＝enable,0=disable）,这个参数和unitControl参数公用
                        				Bit 2: 辅助功能3
                        				Bit 1: 辅助功能2
                        				Bit 0: 辅助功能1*/    
    UInt8   cSpecialFun;                     /* 2.6.3.4特殊功能
...
typedef struct {
    ...
    UInt8 nTurnArray[NUM_PHASE];             /*2.8.3.3 相序组*/
...
typedef struct {
    PhaseItem					stPhase[NUM_PHASE];                                     //相位表,其中也定义了可以并发的相位ID
    ...
    SchemeItem					stScheme[NUM_SCHEME];                                   //方案表
    ...
    ChannelItem					stChannel[NUM_CHANNEL];                                 //通道表
    FollowPhaseItem				stFollowPhase[NUM_FOLLOW_PHASE];                        //跟随相位
    ...
    struct STRU_SignalTransEntry	    AscSignalTransTable[NUM_PHASE];                   //信号转换序列表,其中定义了相位绿闪时间
...
extern Boolean LoadDataFromCfg(SignalControllerPara *pSignalControlpara, const char *path);


/*Read*/
extern void ReadUnitPara(PUnitPara item);
extern void ReadPhaseItem(PPhaseItem item, struct STRU_SignalTransEntry *entry, int num);
extern void ReadChannelItem(PChannelItem item, int num);
extern void ReadGreenSignalRationItem(PGreenSignalRationItem item, int nGreenSignalRationId,int nPhaseNum);
extern void ReadPhaseTurnItem(PhaseTurnItem *item,int nCircleId, int num);
extern void ReadSchemeItem(PSchemeItem item, int num);
extern void ReadActionItem(PActionItem item, int num);
extern void ReadTimeIntervalItem(TimeIntervalItem *item, int nTimeIntervalId, int nTime);
extern void ReadPlanSchedule(PPlanScheduleItem item, int num);
extern void ReadFollowPhaseItem(PFollowPhaseItem item, int num);
extern void ReadVehicleDetector(struct STRU_N_VehicleDetector *item, int num);
extern void ReadPedestrianDetector(struct STRU_N_PedestrianDetector *item, int num);


/*Write*/
extern Boolean WriteConfigFile(SignalControllerPara *param, const char *path);
extern void WriteUnitPara(PUnitPara item);
extern void WritePhaseItem(PPhaseItem item, struct STRU_SignalTransEntry *entry);
extern void WritePhaseTurnItem(PhaseTurnItem (*table)[NUM_RING_COUNT]);
extern void WriteGreenSignalRationItem(GreenSignalRationItem (*table)[NUM_PHASE]);
extern void WriteChannelItem(PChannelItem item);
extern void WriteSchemeItem(PSchemeItem item);
extern void WriteActionItem(PActionItem item);
extern void WriteTimeIntervalItem(TimeIntervalItem (*table)[NUM_TIME_INTERVAL_ID]);
extern void WritePlanSchedule(PPlanScheduleItem item);
extern void WriteFollowPhaseItem(PFollowPhaseItem item);
extern void WriteVehicleDetector(struct STRU_N_VehicleDetector *item);
