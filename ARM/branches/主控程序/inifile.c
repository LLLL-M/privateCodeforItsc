...
unsigned int g_car_detect_switch;             //车检板开关:0不存在，1存在
...
/*********************************************************************************
*
* 	从配置文件获取车检板是否存在(0不存在，1存在)，打印信息是否开启
*
***********************************************************************************/
void get_new_switch_params()
{
	const char * file = "/home/config.ini" ;
    const char * section = "newSwitch";

	g_car_detect_switch = read_profile_int(section, "CarDetectSwitch", 0, file);//车检板是否存在
	printf("car_detect_board:%d\n",g_car_detect_switch);
	g_printf_switch = read_profile_int(section, "PrintfLogSwitch", 0, file);//printf打印信息是否开启
	printf("PrintfLogSwitch:%d\n",g_printf_switch);
}
/*********************************************************************************
*
* 	从配置文件获取所有配置参数
