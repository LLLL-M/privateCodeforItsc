...
#define MSG_CAMERA_FLOW_SERVER_SET	0xe7	//相机流量上传服务器信息设置

#define MSG_TOUCH_CONFIG_APP	0x777777fd	//触摸屏配置工具消息
...
typedef struct SPECIAL_PARAMS
{
	...
	int singleSpotCheckTime;                        //单点优化的检测时间
}STRU_SPECIAL_PARAMS;
...
//单点自适应功能信号机设备端定制
//(接收并上传相机流量,使用的是相机端网络SDK协议,即SDK V30结构)
#define MAX_SDK_SERVER	16			//支持最大16个服务器
#define MSG_UPLOAD_CAMERA_FLOW	6	//sdk要求和其值定义保持一致，故定义为6
#define MSG_ORDERED	1
#define NO_MSG_ORDERED	0
typedef struct CAMERA_FLOW_SERVER_PARAMS
{	char sAddress[16];			//服务器(SDK) ip地址
	unsigned int iPort;			//上传服务器(SDK) 端口
	unsigned int iSDKID;		//SDK维护的id，信号机不需要，sdk发过来，返回消息再带过去
	unsigned char uMsgType;		//订阅消息类型
	unsigned char uIsOrder;		//是否订阅标志(同时也当作新增和删除配置标志)
}STRU_CAMERA_FLOW_SERVER_PARAMS;

//触摸屏配置程序消息定义
#define TOUCH_CONFIG_UPLOAD	0xfa		//上载消息
#define TOUCH_CONFIG_DOWNLOAD	0xfd		//下载消息
typedef struct {
    unsigned short id;		//下载标识，用于区分多设备同时下载
    unsigned char flag;		//上下载标志
    unsigned char type;		//配置消息类型
    unsigned int len;		//数据长度
    unsigned char data[26400-8];	//数据内容
}STRU_TOUCH_BOARD_MSG;
typedef enum {
   TC_MSG_CONFIG_PHASE = 0xa0,
   TC_MSG_CONFIG_CHANNEL = 0xa1,
   TC_MSG_CONFIG_SCHEME = 0xa2,
   TC_MSG_CONFIG_PLAN  = 0xa3,
   TC_MSG_CONFIG_DATE = 0xa4,
   TC_MSG_CONFIG_CARDET = 0xa5,
   TC_MSG_CONFIG_UNIT = 0xa6,
   TC_MSG_CONFIG_ALL = 0xea
}E_Touch_Board_Config_Msg_definition;

#define TOUCH_CONFIG_OP_SUCC	1
#define TOUCH_CONFIG_OP_FAIL	0
