...
void WriteActionItem(PActionItem item)
{
	...
	for(i = 0; i < NUM_ACTION; i++)
	{
		...
		add_one_key(section, "nPhaseTableID", item[i].nPhaseTableID);
		add_one_key(section, "nChannelTableID", item[i].nChannelTableID);
...
*****************************************************************************/
void WriteChannelItem(PChannelItem item,int no)
{
	int i;
	char section[MAX_SECTION_LEN];
	...
	{
		memset(section, 0, MAX_SECTION_LEN); 
		sprintf(section, "%s%d_%d", "ChannelItem",no, item[i].nChannelID);
 ...
 修改历史  
  ...
  3.日    期   : 2015年3月18日
    ...
    修改内容   : 判断相位是否为空的方法改为相位是否使能，只有相位使能后才写
                 入文件中。
*****************************************************************************/
void WritePhaseItem(PPhaseItem item, struct STRU_SignalTransEntry *entry,int no)
{
	int i, n;
	char section[MAX_SECTION_LEN];
	unsigned char array[MAX_ARRAY_LEN];
	...
	{
		memset(section, 0, MAX_SECTION_LEN); 
		sprintf(section, "%s%d_%d", "PhaseItem",no, item[i].nPhaseID);
...
*****************************************************************************/
void WriteFollowPhaseItem(PFollowPhaseItem item,int no)
{
	int i, n;
	char section[MAX_SECTION_LEN];
	unsigned char array[MAX_ARRAY_LEN];
	...
	{
		memset(section, 0, MAX_SECTION_LEN); 
		sprintf(section, "%s%d_%d", "FollowPhaseItem",no, item[i].nFollowPhaseID);
...
Boolean WriteConfigFile(SignalControllerPara *param, const char *path)
{
    ...
    {
        WritePhaseItem(param->stPhase[i], param->AscSignalTransTable[i],i+1);	//写相位表
    }

    for(i = 0 ; i < MAX_CHANNEL_TABLE_COUNT; i++)
    {
        WriteChannelItem(param->stChannel[i],i+1);	//写通道表
    }
    ...
    {
        WriteFollowPhaseItem(param->stFollowPhase[i],i+1);	//写跟随相位表
