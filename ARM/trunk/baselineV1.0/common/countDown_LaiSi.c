...
*****************************************************************************/
void PrintData(unsigned char *nArray)
{
    int i = 0;
...
static void SetCountValue(unsigned char iTempPassTime[2],unsigned char cDeviceId)
{
	...
	if((gStructBinfileCustom.cIsCountdownValueLimit == 1) && (gStructBinfileCustom.sCountdownParams.iFreeGreenTime < cPhaseCountDownTime))//如果某相位的倒计时时间比感应检测时间大，则将显示时间清零，不予显示。如果需要实时显示倒计时状态，那么可以通过配置工具，将单元参数中的感应检测时间调大些。
	{
		iTempPassTime[0] = 0;
		iTempPassTime[1] = 0;
	}
...
8. 每秒钟调用一次即可。
*****************************************************************************/
static void countDownByLaiSiProtocol()
{
    int i =0;
...
*****************************************************************************/
void LaiSiCountDownRun()
{
    SetCountDownValue(gCountDownParams);//每秒更新所有相位的倒计时信息
    
    countDownByLaiSiProtocol(); //根据莱斯协议，计算每个倒计时牌应该显示的倒计时信息
