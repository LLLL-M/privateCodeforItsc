...
static inline void StorePhaseTurn(void *arg)
{
    ...
    {
        memcpy((unsigned char *)&(gConfigPara->stPhaseTurn[item[i].nPhaseTurnID - 1][item[i].nCircleID - 1]), (unsigned char *)&item[i], 18);//这里的相位数目不是16，而是32，
    }
...
void StoreComplete()
{
	...
	ret = 0;//暂时相序表测试出来有错乱的情况，先不进行校验，直接更新全局变量、
	if (ret != 0)
	{
		ERR("the local config is invalid, ret = %d\n", ret);
		memmove(gConfigPara, gSignalControlpara, sizeof(SignalControllerPara));
	...
	else
	{
		...
		if (BIT(g_DownloadFlag, 16) == 0)	//第17位为0表明不需要写入flash，因而也就不需写入配置文件
			return;
		fp = fopen(CFG_NAME, "w");
		if (fp == NULL)
			return;
