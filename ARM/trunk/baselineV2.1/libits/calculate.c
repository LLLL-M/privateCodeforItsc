...
static void SetStagePhaseInfo(UInt8 stageNum, UInt16 stageLeftTime, LineQueueData *data)
{
	...
	for (i = 0; i < gCalInfo.stageInfos[stageNum - 1].includeNum; i++)
	{
		...
		phaseinfos[phaseId - 1].vechileAutoReq = times->laneLevel; //this attributr is same as vehicleAutoReq flag
		phaseinfos[phaseId - 1].pedestrianDetectorBits = times->pedestrianDetectorBits;
		phaseinfos[phaseId - 1].pedestrianAutoReq = times->pedAutoRequestFlag;
		phaseinfos[phaseId - 1].busPrioPassTime = times->busPrioPassTime;
		phaseinfos[phaseId - 1].scarPassTime = times->scarPassTime;
		memcpy(phaseinfos[phaseId - 1].busDetectorBits, times->busDetectorBits, 4);
		memcpy(phaseinfos[phaseId - 1].scarDetectorBits, times->scarDetectorBits, 4);
		...
		phaseinfos[phaseId - 1].minShortGreen = times->minShortGreen;
...
static void SendSwitchSchemeIDMsg(UINT8 switchSchemeId)
{
	if (gCalInfo.actionId == INDUCTIVE_ACTIONID)
			...
			else if (gCalInfo.actionId == SINGLE_OPTIMIZE_ACTIONID)
				ItsCtlNonblock(AUTO_CONTROL, SINGLE_OPTIMIZE_SCHEMEID, switchSchemeId);  //单点优化控制方案切换
			else if (gCalInfo.actionId == PEDESTRIAN_REQ_ACTIONID)
				ItsCtlNonblock(AUTO_CONTROL, PEDESTRIAN_REQ_SCHEMEID, switchSchemeId);	//行人请求过街控制方案切换
			else if(gCalInfo.actionId == BUS_PRIORITY_ACTIONID)
				ItsCtlNonblock(AUTO_CONTROL, BUS_PRIORITY_SCHEMEID, switchSchemeId);	//公交优先控制方案切换	
			else
...
{	
    ...
    while (1)
    {
		...
		//判断当前的方案是感应或者行人过街，状态可能处于相位绿灯常绿状态
		//提前进行判断方案是否切换
		if ((msg.msgSchemeId == 0 || msg.msgSchemeId == PEDESTRIAN_REQ_SCHEMEID) && (lfq_element_count(gHandle) > AHEAD_OF_TIME))
		{
			pthread_rwlock_rdlock(&gConfigLock);
			FillCalInfo(&gCalInfo, msg.msgSchemeId, msg.msgCalTime);
			if (lastCycleSchemeId != gCalInfo.schemeId)
				SendSwitchSchemeIDMsg(gCalInfo.schemeId);
		...
		{	
			...
			if (gCalInfo.actionId != INDUCTIVE_ACTIONID	
				&& gCalInfo.actionId != SINGLE_OPTIMIZE_ACTIONID
				&& gCalInfo.actionId != BUS_PRIORITY_ACTIONID
				&& gCalInfo.actionId != PEDESTRIAN_REQ_ACTIONID
				&& gCalInfo.schemeId != SINGLE_ADAPT_SCHEMEID
				&& gCalInfo.schemeId != PEDESTRIAN_REQ_SCHEMEID
		...
		SendSwitchSchemeIDMsg(switchSchemeId);
