...
void *thread_monitor_can_recv()
{
    ...
    UINT32 nSleepTime = 5;//如果连续失败，可能是没有接灯控板，这时根据失败次数，延长检测时间，以防止系统崩溃.
	INFO("thread_monitor_can_recv thread start ...\n");
    while(1)
    {
        nTempValue = gCanRecvCount;
        //INFO("%d ~~~~~~~~   %d\n",gCanRecvCount,nTempValue);
        usleep(nSleepTime*100*1000);
        //INFO("%d ----   %d   %d \n",gCanRecvCount,nTempValue,(nTempValue == gCanRecvCount));
        if(nTempValue == gCanRecvCount)//如果过了2秒钟，CAN接收计数仍没有变化，那说明CAN有可能挂死了，就重启之
        {
          //  INFO("I will restart can0 %d  %d   --> %d.\n",gCanRecvCount,nTempValue,(nTempValue == gCanRecvCount));
            nErrorCount++;
            if(nErrorCount <= 2)
            {
                CanRestart();
            ...
            {
                if(gStructBinfileMisc.cIsCanRestartHiktscAllowed == 1)//默认情况下是不允许这样做的，除非在配置文件中进行配置。
                {   
                    HikTSCRestart();
                }  
                else
                {
                    INFO("Can may be still error , unless you reboot system .");
                    nErrorCount = 0;
...
void canits_create_recv_thread()
{
	...
	{
		ERR("canits_create_recv_thread create failed  %s. \n",strerror(errno));
	}
}
...
void i_can_its_send_led_request(int boardNum, unsigned short *poutLamp)
{
	...
	//INFO("==> 0x%x 0x%x  0x%x\n",m_frame_send.data[1],m_frame_send.data[2],m_frame_send.data[3]);
}
...
//		INFO("leftlen(%ld)=dwLength(%ld)-sizeof(INTER_DVR_REQUEST_HEAD_V30)(%ld)\n",leftlen,alarmuphead.dwLength,sizeof(INTER_DVR_REQUEST_HEAD_V30));
