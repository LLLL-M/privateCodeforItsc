...
static inline void LCBconfigSet(SignalControllerPara *para, LCBconfig *config)
{
	...
	baseinfo->isTakeOver = gStructBinfileConfigPara.sSpecialParams.iPhaseTakeOverSwtich;
...
void SendRunInfoTOBoard(LineQueueData *data, SignalControllerPara *para)
{
	...
	{
    #if 0
        INFO("i = %d , phasestatus = %d , pedphasestatus = %d, pedChannelBits = %d",i,data->phaseInfos[i].phaseStatus, \
            data->phaseInfos[i].pedestrianPhaseStatus,data->phaseInfos[i].pedChannelBits);
	#endif
        status = data->phaseInfos[i].phaseStatus;
		if (status == GREEN)
			lightvalue = ((data->phaseInfos[i].pedChannelBits != 0) && (data->phaseInfos[i].pedestrianPhaseStatus == GREEN_BLINK)) ? LGREEN_FLASH_PED : LGREEN;
    ...
    #if 0
    INFO("@@@@@ runinfo.schemeid = %d ,runinfo.runtime = %d, "
         "runinfo.phaseR1 = %d, runinfo.lightvalueR1 = %d, "
         "runinfo.phaseR2 = %d, runinfo.lightvalueR2 = %d\n", \
        runinfo.schemeid, runinfo.runtime, \
        runinfo.phaseR1, runinfo.lightvalueR1, \
        runinfo.phaseR2, runinfo.lightvalueR2);
    #endif
}
