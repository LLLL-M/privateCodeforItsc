...
Detector::Detector() : beginTime(time(nullptr))//, log(Singleton<Log>::GetInstance())
{
	status = NORMAL;
}
...
{	//车辆通过车检器时间超过最大持续时间则说明该车检器出现最大持续时间故障了
	...
	for (int i = 0; i < sec; i++, t++)
	{
		...
		auto it = std::find_if(itt.first, itt.second, [sec](VT &ele)->bool{return ele.second > sec;});
...
void Detector::OccupancyRate(time_t t, const int sec)
{
	...
	for (int i = 0; i < sec; i++, t++)
	{
		...
		auto it = std::find_if(itt.first, itt.second, [](VT &ele)->bool{return ele.second > 0;});
...
int Detector::Speed(time_t t, const int sec, const int length)
{
	...
	{
		auto itt = enter.equal_range(t);
		count += std::count_if(itt.first, itt.second, [&sum](VT &ele)->bool{
			sum += ele.second;
			return ele.second > 0;
		});
	}
...
DetectorArray::DetectorArray() : hasFault(0)
{
	
}

DetectorBits DetectorArray::GetEnter(time_t t, int sec)
{
	DetectorBits value(0);
	for (int i = 0; i < MAX_VEHDETECTOR_NUM; i++)
	{
		if (!hasFault[i] && detectors[i].HasEnter(t, sec))
			value[i] = true;	//检测器无故障并且有过车时对应bit置位
	}
	return value;
}


bool DetectorArray::Normal(const DetectorBits &vehDetectorBits)
{
	return (vehDetectorBits & hasFault).any();
}
#if 0
void DetectorArray::Check(const DetectorItem (&table)[MAX_VEHDETECTOR_NUM])
{
	for (int i = 0; i < MAX_VEHDETECTOR_NUM; i++)
	{
		if (table[i].detectorId == (i + 1))
			hasFault[i] = !detectors[i].Check(table[i]);
	}
}
...
vector<TrafficFlowInfo> DetectorArray::GetTrafficInfo(const DetectorItem (&table)[MAX_VEHDETECTOR_NUM], time_t t, const int sec)
{
