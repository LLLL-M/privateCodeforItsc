...
*
***********************************************************************************/
#define BIT(v, n) ((v >> n) & 0x1)	//取v的第n bit位
int DoorCheck()
{
	return 0;
}
unsigned char g_last_key=1;//default auto
unsigned char WirelessKeyCheck(void)
{
	char tmpint=-1;
	unsigned char key=0;
	...
	//开机启动30s内无线遥控不起效
	if((now-system_begin_time) < WIRELESS_CONTROLLER_DELAY_TIME)
		return 0;
	...
	if(fd != -1)
	{
		...
		if(BIT(tmpint, 0) == 0)
		{
			INFO("---> Key 4 AllRed Pressed...");
			key = 4;
		}
		else if(BIT(tmpint, 1) == 0)
		{
			INFO("---> Key 3 YellowBlink Pressed...");
			key = 3;
		}
		else if(BIT(tmpint, 2) == 0)
		{
			INFO("---> Key 5 StepByStep Pressed...");
			key = 5;
		}
		else if(BIT(tmpint, 3) == 0)
		{
			key = 1;
			INFO("---> Key 1 Auto Pressed...");
		}
		else if(BIT(tmpint, 4) == 0)
		{
			key = 2;
			INFO("---> Key 2 Manual Pressed...");
		}
	}

	if(key == lastPressed && abs(now-last)<3)//防抖,在3s内按下相同的按键不会发送
		key = 0;
...
unsigned char keyCheck(int key)
{
	unsigned char ret = 0;
	if(key == 0)
		return 0;
