...
{
	char cmd[128] = {0};
	if (strcmp(dbfile, DATABASE_EXTCONFIG) == 0)
		sprintf(cmd, "cp %s %s", dbfile, DATABASE_EXTCONFIG_BAK);
	else if (strcmp(dbfile, DATABASE_HIKCONFIG) == 0)
		sprintf(cmd, "cp %s %s", dbfile, DATABASE_HIKCONFIG_BAK);
	system(cmd);
...
TableInfo custom_tables[] = {
	...
	{TABLE_NAME_BUS_DETECTOR, "create table %s (id INTEGER PRIMARY KEY, \
					byBusDetectorNumber integer, byBusDetectorCallChannel integer, byBusTagID integer, \
					byRFIDReaderId integer, passTime integer);"},
	{TABLE_NAME_SPECIALCAR_DETECTOR, "create table %s (id INTEGER PRIMARY KEY, \
					bySCarDetectorNumber integer, bySCarDetectorCallChannel integer, bySCarTagID integer, \
					byRFIDReaderId integer, passTime integer, SpecialLevel integer, specialCarCheckSwitch integer);"},
	{TABLE_NAME_CUSTOM_CAMERASERVER, "create table %s (id INTEGER PRIMARY KEY, \
					sAddress varchar(16), iPort integer, iSDKID integer, uMsgType integer, uIsOrder integer);"}
	};
TableInfo desc_tables[] = {
	...
	{TABLE_NAME_DESC_FOLLOW_PHASE, "create table %s (id INTEGER PRIMARY KEY, \
                    followPhaseTableNo integer, followPhaseID integer, followPhaseDescText varchar(64));"},
...
TableInfo hikconfig_tables_name[] = {
	{TABLE_NAME_UNIT, "create table %s (id INTEGER PRIMARY KEY, \
					...
					byFluxCollectCycle integer, byCollectCycleUnit integer, pedKeyDelayTime integer, MaxWaitTime integer, pedKeyPassGap integer);"},
	...
	{TABLE_NAME_PEDESTRIAN, "create table %s (id INTEGER PRIMARY KEY, \
					byPedestrianDetectorNumber integer, byPedestrianDetectorCallPhase integer, byPedestrianDetectorNoActivity integer, \
					byPedestrianDetectorMaxPresence integer, byPedestrianDetectorErraticCounts integer);"},
...
void drop_all_tables()
{
	...
	for (i = 0; i < (sizeof(ip_infos_table) / sizeof(TableInfo)); i++)
	{
		sqlite3_drop_table_wrapper(pdatabase, ip_infos_table[i].name);
	}
	for (i = 0; i < (sizeof(countdown_cfg_tables) / sizeof(TableInfo)); i++)
	{
		sqlite3_drop_table_wrapper(pdatabase, countdown_cfg_tables[i].name);
...
int sqlite3_insert_specialparams(sqlite3* pdatabase, STRU_SPECIAL_PARAMS* pspecial_param)
{
	...
	char* insert_sql = "insert into config_sSpecialParams (iErrorDetectSwitch, iCurrentAlarmSwitch, iVoltageAlarmSwitch, \
					...
					iGpsSwitch, iSignalMachineType, iRedSignalCheckSwitch, iPhaseTakeOverSwtich, isCameraKakou, singleSpotCheckTime) \
					values (?,?,?,?,?,?,?,?,?,?,?,?);";
	...
	ret = sqlite3_bind_int(stmt, 12, pspecial_param->singleSpotCheckTime);
...
int sqlite3_select_specialparams(sqlite3* pdatabase, STRU_SPECIAL_PARAMS* pspecial_param)
{
	...
	while (ret == SQLITE_ROW)
	{
		...
		pspecial_param->singleSpotCheckTime = sqlite3_column_int(stmt, 12);
		ret = sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
	return 0;
...
int sqlite3_insert_bus_detector(sqlite3* pdatabase, STRU_N_BusDetector *pbus)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	char* insert_sql = "insert into BusDetector (byBusDetectorNumber, byBusDetectorCallChannel, byBusTagID, \
					byRFIDReaderId, passTime) values (?,?,?,?,?);";
	...
	{
		if (pbus[i].byBusDetectorCallChannel == 0)
			continue;
		sqlite3_reset(stmt);
		sqlite3_clear_bindings(stmt);
		
		ret = sqlite3_bind_int(stmt, 1, pbus[i].byBusDetectorNumber);
		ret = sqlite3_bind_int(stmt, 2, pbus[i].byBusDetectorCallChannel);
		ret = sqlite3_bind_int(stmt, 3, pbus[i].byBusTagID);
		ret = sqlite3_bind_int(stmt, 4, pbus[i].byRFIDReaderId);
		ret = sqlite3_bind_int(stmt, 5, pbus[i].passTime);
		ret = sqlite3_step(stmt);
	    if(ret != SQLITE_DONE)
		{
			ERR("insert BusDetector  failed,err=%s\n", sqlite3_errmsg(pdatabase));
			sqlite3_finalize(stmt);
			return -1;
	    }
	}
...
int sqlite3_select_bus_detector(sqlite3* pdatabase, STRU_N_BusDetector *pbus)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	
	char select_sql[] = {"select * from BusDetector;"};
	...
	while (ret == SQLITE_ROW)
	{
		pbus[i].byBusDetectorNumber = sqlite3_column_int(stmt, 1);
		pbus[i].byBusDetectorCallChannel = sqlite3_column_int(stmt, 2);
		pbus[i].byBusTagID = sqlite3_column_int(stmt, 3);
		pbus[i].byRFIDReaderId = sqlite3_column_int(stmt, 4);
		pbus[i].passTime = sqlite3_column_int(stmt, 5);
		i++;
		ret = sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
...
int sqlite3_insert_specialcar_detector(sqlite3* pdatabase, STRU_N_SpecialCarDetector *pspecialcar)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	char* insert_sql = "insert into SpecialCarDetector (bySCarDetectorNumber, byScarDetectorCallChannel, byScarTagID, \
					byRFIDReaderId, passTime, specialLevel) values (?,?,?,?,?,?);";
	...
	for (i = 0; i < MAX_BUSDETECTOR_NUM; i++)
	{
		if (pspecialcar[i].bySCarDetectorCallChannel == 0)
			continue;
		sqlite3_reset(stmt);
		sqlite3_clear_bindings(stmt);
		
		ret = sqlite3_bind_int(stmt, 1, pspecialcar[i].bySCarDetectorNumber);
		ret = sqlite3_bind_int(stmt, 2, pspecialcar[i].bySCarDetectorCallChannel);
		ret = sqlite3_bind_int(stmt, 3, pspecialcar[i].bySCarTagID);
		ret = sqlite3_bind_int(stmt, 4, pspecialcar[i].byRFIDReaderId);
		ret = sqlite3_bind_int(stmt, 5, pspecialcar[i].passTime);
		ret = sqlite3_bind_int(stmt, 6, pspecialcar[i].specialLevel);
		ret = sqlite3_step(stmt);
	    if(ret != SQLITE_DONE)
		{
			ERR("insert SpecialCarDetector  failed,err=%s\n", sqlite3_errmsg(pdatabase));
			sqlite3_finalize(stmt);
			return -1;
	    }
	}
...
int sqlite3_select_specialcar_detector(sqlite3* pdatabase, STRU_N_SpecialCarDetector *pspecialcar)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	
	char select_sql[] = {"select * from SpecialCarDetector;"};
	...
	while (ret == SQLITE_ROW)
	{
		pspecialcar[i].bySCarDetectorNumber = sqlite3_column_int(stmt, 1);
		pspecialcar[i].bySCarDetectorCallChannel = sqlite3_column_int(stmt, 2);
		pspecialcar[i].bySCarTagID = sqlite3_column_int(stmt, 3);
		pspecialcar[i].byRFIDReaderId = sqlite3_column_int(stmt, 4);
		pspecialcar[i].passTime = sqlite3_column_int(stmt, 5);
		pspecialcar[i].specialLevel= sqlite3_column_int(stmt, 6);
		i++;
		ret = sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
...
int sqlite3_select_cameraServer_prm(sqlite3* pdatabase, STRU_CAMERA_FLOW_SERVER_PARAMS* pserver_param)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	
	char select_sql[] = {"select * from custom_sCameraServParams;"};
	...
	while (ret == SQLITE_ROW)
	{
		memcpy(pserver_param[i].sAddress, sqlite3_column_text(stmt, 1), 16);
		pserver_param[i].iPort = sqlite3_column_int(stmt, 2);
		pserver_param[i].iSDKID = sqlite3_column_int(stmt, 3);
		pserver_param[i].uMsgType = sqlite3_column_int(stmt, 4);
		pserver_param[i].uIsOrder = sqlite3_column_int(stmt, 5);
		i++;
		if (i >= MAX_SDK_SERVER)
			break;
		ret = sqlite3_step(stmt);
...
int sqlite3_insert_cameraServer_prm(sqlite3* pdatabase, STRU_CAMERA_FLOW_SERVER_PARAMS* pserver_param)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	char* insert_sql = "insert into custom_sCameraServParams (sAddress, iPort, iSDKID, uMsgType, uIsOrder) \
					values (?,?,?,?,?);";
	...
	for (i = 0; i < MAX_SDK_SERVER; i++)
	{
		sqlite3_reset(stmt);
		sqlite3_clear_bindings(stmt);
		ret = sqlite3_bind_text(stmt, 1, pserver_param[i].sAddress, 16, NULL);
		ret = sqlite3_bind_int(stmt, 2, pserver_param[i].iPort);
		ret = sqlite3_bind_int(stmt, 3, pserver_param[i].iSDKID);
		ret = sqlite3_bind_int(stmt, 4, pserver_param[i].uMsgType);
		ret = sqlite3_bind_int(stmt, 5, pserver_param[i].uIsOrder);

		ret = sqlite3_step(stmt);
	    if(ret != SQLITE_DONE)
		{
			ERR("insert custom_sCameraServParams  failed,err=%s\n", sqlite3_errmsg(pdatabase));
			sqlite3_finalize(stmt);
			return -1;
	    }
	}
...
int read_custom(sqlite3* pdatabase, STRUCT_BINFILE_CUSTOM* pcustom)
{
	...
	ret += sqlite3_select_bus_detector(pdatabase, pcustom->sBusDetector);
	ret += sqlite3_select_specialcar_detector(pdatabase, pcustom->sSpecialCarDetector);
	ret += sqlite3_select_column(pdatabase, TABLE_NAME_SPECIALCAR_DETECTOR, "specialCarCheckSwitch", 1, &(pcustom->specialCarCheckSwitch), 1);
	ret += sqlite3_select_cameraServer_prm(pdatabase, pcustom->sCameraFlowServer);
	return (ret == 0) ? 0 : -1;
}
int write_custom(sqlite3* pdatabase, STRUCT_BINFILE_CUSTOM* pcustom)
{
	...
	ret += sqlite3_insert_bus_detector(pdatabase, pcustom->sBusDetector);
	ret += sqlite3_insert_specialcar_detector(pdatabase, pcustom->sSpecialCarDetector);
	ret += sqlite3_update_column(pdatabase, TABLE_NAME_SPECIALCAR_DETECTOR, "specialCarCheckSwitch", 1, &(pcustom->specialCarCheckSwitch), 1, SQLITE_INTEGER);
	ret += sqlite3_insert_cameraServer_prm(pdatabase, pcustom->sCameraFlowServer);
...
//数据库中插入跟随相位描述，缅甸项目定制； jgp
int sqlite3_insert_followphase_desc(sqlite3* pdatabase, PhaseDescText followphase_desc[16][16])
{
	int ret = 0;
	int i = 0;
	int j = 0;
	sqlite3_stmt *stmt = NULL;
	char* insert_sql = "insert into desc_followPhaseDescText (followPhaseTableNo, followPhaseID, followPhaseDescText) values (?,?,?);";
	...
	for (i = 0; i < MAX_PHASE_TABLE_COUNT; i++)
	{
		for (j = 0; j < NUM_PHASE; j++)
		{
			if (0 == strlen((char*)followphase_desc[i][j]))
				continue;
			...
			ret = sqlite3_bind_int(stmt, 1, i + 1);
			ret = sqlite3_bind_int(stmt, 2, j + 1);
			ret = sqlite3_bind_text(stmt, 3, (const char*)(followphase_desc[i][j]), 64, NULL);
			ret = sqlite3_step(stmt);
	    	if(ret != SQLITE_DONE)
			{
				ERR("insert column followPhaseDescText[%d][%d] value failed,err=%s\n", i, j, sqlite3_errmsg(pdatabase));
				sqlite3_finalize(stmt);
				return -1;
	    	}	
		}
...
//从数据库中读取跟随相位描述到全局变量，缅甸项目定制；jgp
int sqlite3_select_followphase_desc(sqlite3* pdatabase, PhaseDescText followphase_desc[16][16])
{
	int ret = 0;
	int i = 0;
	int j = 0;
	...
	char select_sql[] = {"select followPhaseTableNo, followPhaseID, followPhaseDescText from desc_followPhaseDescText;"};
	...
	while (ret == SQLITE_ROW)
	{
		...
		i = sqlite3_column_int(stmt, 0);
		j = sqlite3_column_int(stmt, 1);
		memcpy(followphase_desc[i-1][j-1], sqlite3_column_blob(stmt, 2), sqlite3_column_bytes(stmt, 2));
...
int read_desc(sqlite3* pdatabase, STRUCT_BINFILE_DESC *pdesc)
{
	...
	ret += sqlite3_select_followphase_desc(pdatabase, pdesc->followPhaseDescText);
...
int write_desc(sqlite3* pdatabase, STRUCT_BINFILE_DESC *pdesc)
{
	...
	ret += sqlite3_insert_followphase_desc(pdatabase, pdesc->followPhaseDescText); 
...
int sqlite3_insert_phasepasstimes(sqlite3* pdatabase, int* phasepasstimes)
{
	return sqlite3_update_column(pdatabase, TABLE_NAME_MISC, "iPhasePassTimes", 1, phasepasstimes, 4, SQLITE_INTEGER);
}
int sqlite3_select_phasepasstimes(sqlite3* pdatabase, int* phasepasstimes)
{
	return sqlite3_select_column(pdatabase, TABLE_NAME_MISC, "iPhasePassTimes", 1, phasepasstimes, 4);
}
...
static int sqlite3_update_pedparams(sqlite3* pdatabase, UnitPara *punit)
{
	int ret = 0;
	sqlite3_stmt *stmt = NULL;
	char update_sql[256] = {0};
	sprintf(update_sql, "update stUnitPara set pedKeyDelayTime=%d, maxWaitTime=%d, pedKeyPassGap=%d where ROWID=1;", 
		punit->pedKeyDelayTime, punit->maxWaitTime, punit->pedKeyPassGap);
...
int sqlite3_insert_unit(sqlite3* pdatabase, UnitPara *punit)
{
	...
	//ret = sqlite3_bind_int(stmt, 7, punit->pedKeyDelayTime);
	//ret = sqlite3_bind_int(stmt, 8, punit->maxWaitTime);
	//ret = sqlite3_bind_int(stmt, 9, punit->pedKeyPassGap);
	...
	sqlite3_finalize(stmt);
	ret = sqlite3_update_pedparams(pdatabase, punit);
	if (ret == -1)
		ERR("update pedestrian params failed!");
	return 0;
}
...
int sqlite3_select_unit(sqlite3* pdatabase, UnitPara *punit)
{
	...
	while (ret == SQLITE_ROW)
	{
		...
		punit->pedKeyDelayTime = sqlite3_column_int(stmt, 7);
		punit->maxWaitTime = sqlite3_column_int(stmt, 8);
		punit->pedKeyPassGap = sqlite3_column_int(stmt, 9);
		ret = sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
	return 0;
...
int sqlite3_select_vehicle(sqlite3* pdatabase, struct STRU_N_VehicleDetector *pvehicle)
{
	...
	while (ret == SQLITE_ROW)
	{
		if (i > MAX_VEHICLEDETECTOR_COUNT)
			break;
		i = sqlite3_column_int(stmt, 1);
		if (i > 0 && i <= MAX_VEHICLEDETECTOR_COUNT)
		{
...
int sqlite3_insert_pedestrian(sqlite3* pdatabase, struct STRU_N_PedestrianDetector *ppedestrian)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	char* insert_sql = "insert into AscPedestrianDetectorTable (byPedestrianDetectorNumber, byPedestrianDetectorCallPhase, \
						byPedestrianDetectorNoActivity, byPedestrianDetectorMaxPresence, byPedestrianDetectorErraticCounts) values (?,?,?,?,?);";
	...
	{
		//INFO("vehicle num=%d, callphase=%d, switchphase=%d", pvehicle[i].byVehicleDetectorNumber, pvehicle[i].byVehicleDetectorCallPhase, pvehicle[i].byVehicleDetectorSwitchPhase);
		if (ppedestrian[i].byPedestrianDetectorCallPhase == 0)
			continue;
		
		sqlite3_reset(stmt);
		sqlite3_clear_bindings(stmt);
		ret = sqlite3_bind_int(stmt, 1, ppedestrian[i].byPedestrianDetectorNumber);
		ret = sqlite3_bind_int(stmt, 2, ppedestrian[i].byPedestrianDetectorCallPhase);
		ret = sqlite3_bind_int(stmt, 3, ppedestrian[i].byPedestrianDetectorNoActivity);
		ret = sqlite3_bind_int(stmt, 4, ppedestrian[i].byPedestrianDetectorMaxPresence);
		ret = sqlite3_bind_int(stmt, 5, ppedestrian[i].byPedestrianDetectorErraticCounts);
		
		ret = sqlite3_step(stmt);
		if(ret != SQLITE_DONE)
		{
			ERR("insert AscPedestrianDetectorTable[%d] failed,err=%s\n", i, sqlite3_errmsg(pdatabase));
			sqlite3_finalize(stmt);
			return -1;
		}
	}
...
int sqlite3_select_pedestrian(sqlite3 * pdatabase,struct STRU_N_PedestrianDetector * ppedestrian)
{
	int ret = 0;
	int i = 0;
	sqlite3_stmt *stmt = NULL;
	
	char select_sql[] = {"select * from AscPedestrianDetectorTable;"};
	...
	while (ret == SQLITE_ROW)
	{
		if (i > MAX_PEDESTRIANDETECTOR_COUNT)
			break;
		i = sqlite3_column_int(stmt, 1);
		if (i > 0 && i <= MAX_PEDESTRIANDETECTOR_COUNT)
		{
			ppedestrian[i-1].byPedestrianDetectorNumber = sqlite3_column_int(stmt, 1);
			ppedestrian[i-1].byPedestrianDetectorCallPhase = sqlite3_column_int(stmt, 2);
			ppedestrian[i-1].byPedestrianDetectorNoActivity = sqlite3_column_int(stmt, 3);
			ppedestrian[i-1].byPedestrianDetectorMaxPresence = sqlite3_column_int(stmt, 4);
			ppedestrian[i-1].byPedestrianDetectorErraticCounts = sqlite3_column_int(stmt, 5);
		}
		ret = sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
...
int write_hikconfig(sqlite3* pdatabase, SignalControllerPara* pscp)
{
	...
	ret += sqlite3_insert_pedestrian(pdatabase, pscp->AscPedestrianDetectorTable);
...
int read_hikconfig(sqlite3* pdatabase, SignalControllerPara* pscp)
{
	...
	ret += sqlite3_select_pedestrian(pdatabase, pscp->AscPedestrianDetectorTable);
