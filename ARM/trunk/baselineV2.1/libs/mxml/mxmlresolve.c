...
static char *mxmlGetSimpleNode(mxmlNode *info)
{
	...
	if((info->type == MXML_SIMPLE_STR || info->type == MXML_SIMPLE_ARR) && info->id1 != 0)
	{
			...
			node!=NULL;node=mxmlFindElement(node, info->cur, mxmlLabels[info->type-1], "id", id1, MXML_DESCEND))
		{
			if(strcmp(info->ob, mxmlElementGetAttr(node,"ob")) == 0)
			{
                if(node->child != NULL)
                    ...
                    INFO("%s: NO data under the node.",__func__);
                #endif
...
void mxmlAddNodeStr(mxml_node_t *top, char *ob, char *value, int id)
{
	...
	{
		sprintf(buf, "%d", id);
		for(n1=mxmlFindElement(ptree,ptree, mxmlLabels[1], "ob", ob, MXML_DESCEND);n1!=NULL;
				n1=mxmlFindElement(n1,ptree, mxmlLabels[1], "ob", ob, MXML_DESCEND))
		{
			for(n2=mxmlFindElement(ptree,ptree, mxmlLabels[1], "id", buf, MXML_DESCEND);n2!=NULL;
					n2=mxmlFindElement(n2,ptree, mxmlLabels[1], "id", buf, MXML_DESCEND))
				if(n1 == n2)
					break;
			if(n2 != NULL)
				break;
		...
		n1 = mxmlFindElement(ptree, ptree, mxmlLabels[1], "ob", ob, MXML_DESCEND);
