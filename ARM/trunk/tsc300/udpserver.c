...
*****************************************************************************/
static void UploadFaultInfo(int socketFd,struct sockaddr_in fromAddr)
{
    ssize_t result = 0;
...
void udp_server_process()
{
    ...
    while(1)
    {
		...
		fprintf(stderr,"recv size = %d  type  =  0x%x  %d\n", result,udp_info.iType,udp_info.iType);
        ...
        else
        {
			...
			else if(udp_info.iHead == 0x6e6e && udp_info.iType == 0x15b)//上载故障信息
			{
                UploadFaultInfo(socketFd,fromAddr);
			}
			...
			else if(udp_info.iHead == 0x6e6e && udp_info.iType == 0xa1)//上载软硬件版本
			{
			...
			{
			    s_special_params.iSignalMachineType = udp_info.iValue[0];
			    iIsSaveSpecialParams = 1;
			    SendSuccessMsg(socketFd,fromAddr);
			}
			else if(udp_info.iHead == 0x6e6e && udp_info.iType == 0xbf)//获取经济型信号机型号
			{
			    udp_info.iValue[0] = s_special_params.iSignalMachineType;
				result = sendto(socketFd, &udp_info, 12, 0, (struct sockaddr*)&fromAddr, sizeof(fromAddr));
			}
