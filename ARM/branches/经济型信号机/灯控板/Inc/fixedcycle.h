...
#define LCB_CONFIG_ADDR		(0x0803F800)	//把用户可用的flash地址最后一页用来保存配置
...
static LCBconfig gDefaultConfig = {
	...
	.phaseinfo = {
		[0] = {3, 3, 2, 6, 0x8686},	//相位1:东西直行,时间20s,对应通道2,3,10,11,8,16
...
static inline void WriteFlashData(uint32_t flashAddr, void *buf, int size)
{
	...
	HAL_FLASH_Unlock();
	//先擦除以前的数据
	if (HAL_FLASHEx_Erase(&EraseInitStruct, &pageError) != HAL_OK)	//擦除失败
	...
	{	//擦除成功则把接收的数据存入flash中
		...
		for (i = 0; i < count; i++)
		{
			if (HAL_FLASH_Program(TYPEPROGRAM_WORD, flashAddr + i * sizeof(uint32_t), data[i]) != HAL_OK)
			{
...
static void LCBconfigDeal(CanRxMsgTypeDef *pRxMsg)
{
...
static LightValue GetPhaseLightValue(uint8_t splitTime, uint8_t runtime, LCBphaseinfo *phaseinfo)
{
	...
	int pedGreenTime = greenTime + phaseinfo->greenFlashTime - phaseinfo->pedFlashTime;
...
static inline void LightFrontBoardButtonLed(uint8_t data)
{
	...
	{
		g_CanHandle.pTxMsg->StdId = 0x110;
		g_CanHandle.pTxMsg->ExtId = 0;
		g_CanHandle.pTxMsg->IDE = CAN_ID_STD;//
		g_CanHandle.pTxMsg->RTR = CAN_RTR_DATA;//消息帧类型为数据帧 
		g_CanHandle.pTxMsg->DLC = 1;//帧长度为1
		g_CanHandle.pTxMsg->Data[0] = data;
		Can_Send_Recv_Data(&g_CanHandle, 0);
	}
}
//根据获取的红灯电流值转换为点灯bit值
...
static inline void ConvertToLightData(CanRxMsgTypeDef *pRxMsg)
{
