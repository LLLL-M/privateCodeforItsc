...
static inline void *lfq_read_back_prefetch(void* handle, int block_no)
{
	struct lfq_t* queue = handle;
	if (queue)
	{
		register UInt32 rd = queue->rd, next_rd;
		UInt32 backoffset = queue->element_size * block_no;
		next_rd = (rd > backoffset)? (rd - backoffset) : (queue->queue_size + rd - backoffset);
        return (void *)(queue->data + next_rd);
    }
	else
	{
