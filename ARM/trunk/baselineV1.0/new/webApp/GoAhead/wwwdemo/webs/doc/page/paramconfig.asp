...
function xmlToString(xmlData) {
	...
	function SpecialControl(schemeId)
	{
		var o = {"schemeId":schemeId, "stageNo":"0"};
		var xmlStr = $.json2xml(o,{root:'specialControl'});
		$.ajax({
		  type: 'POST',
		  dataType: 'xml',
		  data: xmlStr,
		  url: m_lHttp+m_szHostName+":"+m_lHttpPort +'/goform/specialControl',
		  success: function(data, textStatus){
				//alertMsg(xmlToString(data));
		  }
		});
	}
	function ChangePassword(password)
	{
		var newpassword = (password == null) ? $("#newpassword").val() : password;
		var o = {"user":$.cookie('username'), "oldpassword":$.cookie('password'), "newpassword":newpassword};
		var xmlStr = $.json2xml(o,{root:'passwordChange'});
		$.ajax({
		  type: 'POST',
		  dataType: 'xml',
		  data: xmlStr,
		  url: m_lHttp+m_szHostName+":"+m_lHttpPort +'/goform/ChangePassword',
		  success: function(data, textStatus){
				alert("密码更改成功，新密码是" + newpassword);
	...
	function GetSplitTable()
	{
		$.ajax({
		  type: 'GET',
		  dataType: 'xml',
		  url: m_lHttp+m_szHostName+":"+m_lHttpPort +'/goform/splitTable',
		  success: function(data, textStatus){
    ...
    $(document).ready(function() {
      ...
      //给按钮设置样式end
	  //给特殊控制按钮设置样式start, added by Jicky
	  $(".control").css({"height":"30px","width":"100px","font-size":"20px","font-weight":"bolder","color":"black"});
	  $(".control").css("background-color", function(index, value) {
			if (index == 0) {
				return 'yellow';
			} else if (index == 1) {
				return 'dimgray';
...
-->
	  ...
	  <tr><td><button class="control" onClick="SpecialControl(255)">黄闪</button></td></tr>
	  <tr><td><button class="control" onClick="SpecialControl(251)">关灯</button></td></tr>
	  <tr><td><button class="control" onClick="SpecialControl(252)">全红</button></td></tr>
	  <tr><td><button class="control" onClick="SpecialControl(249)">步进</button></td></tr>
	  <tr><td><button class="control" onClick="SpecialControl(0)">自动</button></td></tr>
<!--
	  <tr style="height:20px;"></tr>
	  <tr><td>请输入所要更改的密码</td></tr>
	  <tr><td><input type="Password" style="background-color:gray" id="newpassword"/></td></tr>
	  <tr><td><button style="height:30px;background-color:Aqua" onClick="ChangePassword(null)">更改密码</button></td></tr>
	  <tr><td><button style="height:30px;background-color:DodgerBlue" onClick="ChangePassword('123456')">恢复默认密码</button></td></tr>
	  <tr><td><button style="height:30px;background-color:Green" onClick="GetSplitTable()">获取绿信比表</button></td></tr>
-->
  </div>
</body>
</html>
