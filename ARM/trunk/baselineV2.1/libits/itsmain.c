...
static int gThreadNum = 8;	//线程个数需要与上面的个数一一对应
...
static void SendControlMsg(ControlType type, UInt8 schemeId, int val, Boolean isblock)
{
	...
	//INFO("func[%s] recv ITS control, ctltype=%d,  schemeId:%d, val:%d", 
		//__func__, type, schemeId, val);
    memset(&msg, 0, sizeof(msg));
	...
	switch (schemeId)
	{
		...
		case SINGLE_OPTIMIZE_SCHEMEID: msg.msgMode = SINGLE_SPOT_OPTIMIZE; msg.msgmSchemeId = val; break;
		case PEDESTRIAN_REQ_SCHEMEID: msg.msgMode = PEDESTRIAN_REQ_MODE; msg.msgmSchemeId = val; break;
		case BUS_PRIORITY_SCHEMEID: msg.msgMode = BUS_PRIORITY_MODE; msg.msgmSchemeId = val; break;
	...
	//INFO("func[%s] recv ITS control, ctltype=%d, mode:%d schemeId:%d, val:%d", 
		//__func__, msg.msgControlType, msg.msgMode, msg.msgmSchemeId, val);
