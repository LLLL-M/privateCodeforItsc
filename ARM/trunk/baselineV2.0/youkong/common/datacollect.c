...
extern TimeAndHistoryVolume gRealtimeVolume;              //实时流量，只有流量是实时的

static LineQueueData lineData;//用来保存500ms的消息队列内容
...
//通过车检器ID得到其对应的相位ID,两个ID均从1开始,if error occured, 0 returned.
static UInt8 GetPhaseIdByVehicleId(UInt8 nVehicleId,LineQueueData *data)
{
    int i = 0;
    ...
    for(i = 0; i < 16; i++)
    {
        if(((data->phaseInfos[i].vehicleDetectorBits >> (nVehicleId - 1))&0x01) == 0x01)
        {
            return i+1;
        }
    }
...
static void CalcStatData(TimeAndHistoryVolume *timeAndHistoryVolume,UINT32 nCycle)
{
    ...
    for(i = 0; i < 48; i++)
    {
        ...
        else
        {
            ...
            nPhaseId = GetPhaseIdByVehicleId(i+1,&lineData); //找到车检器对应的相位
            nWaitTime = 0;
            if(nPhaseId >= 1 && nPhaseId <= 16)
            {
                nWaitTime = lineData.cycleTime - lineData.phaseInfos[nPhaseId - 1].splitTime;
            }
...
static void VehicleFlowStat(UINT8 boardNum,UINT16 boardInfo)
{
    ...
    UINT16 nCycleTime = lineData.collectCycle;//采集周期，单位是秒
    ...
    for(i = 0; i < 16; i++)
    {
        ...
        {
            gRealtimeVolume.struVolume[(boardNum - 1)*16 + i].byDetectorVolume++;
            nIsHaveCar = 1;
        }

        nPhaseId = GetPhaseIdByVehicleId((boardNum - 1)*16 + i +1,&lineData);
        //如果当前检测器对应的相位是绿灯，且有过车，就计算绿损时间
        if((nPhaseId >=1) && (nPhaseId < 16)&&(lineData.phaseInfos[nPhaseId - 1].phaseStatus == 1))
        {
            if(nFlag == 0)
            {
                nFlag = 1;
            ...
            if(nIsHaveCar == 0)//如果相位是绿灯，但没有过车，那么绿损时间就要加1秒
            {
                ...
                {
                    gRealtimeVolume.struVolume[(boardNum - 1)*16 + i].wGreenLost += 1*100;
                    nFlag = 0;
                }
            }
            else
        ...
        else//红灯时，根据是否已经统计过起始时间来计算时间占有率
        {
            if(nFlagIsCalcOccupancy[(boardNum - 1)*16 + i] == 1)
            {
                ...
                if(nTempLong != 0)
                {
                    gRealtimeVolume.struVolume[(boardNum - 1)*16 + i].byDetectorOccupancy = 100.0*nTimeOccupy[(boardNum - 1)*16 + i]/nTempLong;
                }
    ...
    gRealtimeVolume.dwTime = startTime;
    CalcStatData(&gRealtimeVolume,nowTime - startTime);
    ...
    if(nowTime - startTime >= nCycleTime)
    {
        for(i = 0; i < 48; i++)
        {
            if(nFlagIsCalcOccupancy[i] == 1)
            {
                ...
                if(nTempLong != 0)
                {
                    gRealtimeVolume.struVolume[i].byDetectorOccupancy = 10000.0*nTimeOccupy[i]/(1.0*nTempLong);
                
                    /*if(i == 0)
                    {
                        INFO("===> volumeOccupancy  %0.2f%% \n",gRealtimeVolume.struVolume[i].byDetectorOccupancy/100.0);
                    }*/
                }
                nTimeOccupy[i] = 0;
            }
       ...
       // CalcStatData(&gRealtimeVolume,nowTime - startTime);
        RecordVehicleFlowData(&gRealtimeVolume);

        startTime = nowTime;
        nFlag = 0;
        memset(&gRealtimeVolume, 0, sizeof(TimeAndHistoryVolume));
    }
