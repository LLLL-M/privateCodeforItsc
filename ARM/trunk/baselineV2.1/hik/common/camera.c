...
extern void SendCameraFlow(char *data, int len);
...
static void RecvCameraData(int fd)
{
	...
	INTER_DVR_REQUEST_HEAD_V30 *head = NULL;		//改为读取V30 头部
	...
	char *p = NULL;
	char *pJsonText = NULL;	//SDK网络协议流量数据内容
	...
	//首先读取相机协议头部
	if (StreamRead(stream, sizeof(INTER_DVR_REQUEST_HEAD_V30)) == FALSE)
		return;
	head = (INTER_DVR_REQUEST_HEAD_V30 *)stream->buf;
	//INFO("-->Data msgtype: %d..\n", head->byCommand);
	if(head->byCommand == 0x98)//short connection
	{
		//INFO("len: %d, offset: %d, convert_len: %d...\n", head->dwLength, stream->offset, ntohl(head->dwLength));
		//INFO("-->IPtype: %d, version: %d, dvrname: %s, serialNumber: %s, port: %d", );
		if(head->dwLength > 10 * 1024)//network byte order, sometimes msg can't be read in one time, ' if ' can be judged twice. 
		{
			head->dwLength = ntohl(head->dwLength);
			//ERR("Illegal packet length!");
			//return;
		}
		if(head->dwLength > stream->size && (p = malloc(head->dwLength + 1)) != NULL)
		{
			memset(p, 0, head->dwLength + 1);
			memcpy(p, stream->buf, stream->offset);
			free(stream->buf);
			stream->buf = p;
			stream->size = head->dwLength + 1;
			p = NULL;
		}
		if (StreamRead(stream, head->dwLength) == FALSE)
			return;

		pJsonText = stream->buf + sizeof(INTER_DVR_REQUEST_HEAD_V30);
		//INFO("-->Camera recv json Text: %s\n", pJsonText);
		SendCameraFlow(pJsonText, head->dwLength - sizeof(INTER_DVR_REQUEST_HEAD_V30));
		StreamReset(stream);
	}
	else if (head->byCommand == 0xb6 && head->dwLength == sizeof(NET_TPS_ALARM_REALTIME))	//实时信息
	{
		if (StreamRead(stream, sizeof(NET_TPS_ALARM_REALTIME)) == FALSE)// 8 == sizeof(NET_TPS_ALARM_HEAD) - sizeof(INTER_DVR_REQUEST_HEAD_V30)
			return;
