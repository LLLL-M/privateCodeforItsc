...
void udp_server_process()
{
	...
	STRU_CURRENT_PARAMS s_CurrentParams[32];
    struct Count_Down_Params S_CountDownParams;    //倒计时参数
    PHASE_DESC_PARAMS   s_PhaseDescParams;        //相位描述  
    CHANNEL_DESC_PARAMS channel_desc_params;      //通道描述
    CHANNEL_DESC_PARAMS s_ChannelDescParams;      //通道描述
    PATTERN_NAME_PARAMS s_PatternDescParams;      //方案描述
    PLAN_NAME_PARAMS    s_PlanDescParams;         //计划描述
    DATE_NAME_PARAMS    s_DateDescParams;		  //日期描述
    COM_PARAMS          s_ComParams;              //串口设置参数
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if(1)//接收完成后，保存局部变量到配置文件及更新全局参数。
            {
                //更新看门狗和GPS进程
                if((s_special_params.iWatchdogSwitch == 0) && (s_SpecialParams.iWatchdogSwitch == 1))
                {
					//硬件看门狗从关闭到打开
                ...
                else if((s_special_params.iWatchdogSwitch == 1) && (s_SpecialParams.iWatchdogSwitch == 0))
                {
					//硬件看门狗从打开到关闭
				...
				if((s_special_params.iGpsSwitch == 0) && (s_SpecialParams.iGpsSwitch == 1))
				{
					//GPS从关闭到打开
					system("killall -9 GPS &");
					system("/root/GPS &");
				}
				else if((s_special_params.iGpsSwitch == 1) && (s_SpecialParams.iGpsSwitch == 0))
				{
					//GPS从打开到关闭
					system("killall -9 GPS &");
					Set_LED2_OFF();
                ...
                //保存参数到配置文件
                set_special_params(&s_SpecialParams,s_CurrentParams,g_failurenumber);
                set_custom_params(&S_CountDownParams, &s_ComParams);
                set_desc_params(&s_PhaseDescParams,&s_ChannelDescParams,&s_PatternDescParams,&s_PlanDescParams,&s_DateDescParams);

                //更新全局变量
                memcpy(&s_special_params,&s_SpecialParams,sizeof(s_special_params));
                memcpy(g_struRecCurrent,s_CurrentParams,sizeof(s_CurrentParams));
                memcpy(&g_struCountDown,&S_CountDownParams,sizeof(g_struCountDown));
                //串口的有问题
                memcpy(&phase_desc_params,&s_PhaseDescParams,sizeof(phase_desc_params));
                memcpy(&channel_desc_params,&s_ChannelDescParams,sizeof(s_ChannelDescParams));
                memcpy(&pattern_name_params,&s_PatternDescParams,sizeof(s_PatternDescParams));
                memcpy(&plan_name_params,&s_PlanDescParams,sizeof(s_PlanDescParams));
                memcpy(&date_name_params,&s_DateDescParams,sizeof(s_DateDescParams));
			...
			{
				s_SpecialParams.iErrorDetectSwitch = udp_info.iValue[0]& 0x1;
				s_SpecialParams.iCurrentAlarmSwitch = (udp_info.iValue[0] & 0x2) >> 1;
				s_SpecialParams.iVoltageAlarmSwitch = (udp_info.iValue[0] & 0x4) >> 2;
				s_SpecialParams.iCurrentAlarmAndProcessSwitch = (udp_info.iValue[0] & 0x8) >> 3;
				s_SpecialParams.iVoltageAlarmAndProcessSwitch = (udp_info.iValue[0] & 0x10) >> 4;
				s_SpecialParams.iWatchdogSwitch = ((udp_info.iValue[0] & 0x20) >> 5);
				s_SpecialParams.iGpsSwitch = ((udp_info.iValue[0] & 0x40) >> 6);	
			...
			else if(udp_info.iHead == 0x6e6e && udp_info.iType == 0x96)
			{
				...
				memcpy(s_CurrentParams, udp_info.iValue, 32*8);
				
				for(i=0;i<32;i++)
				{
					DBG("保存通道%02d红灯电流基准值:%03d\n",i+1,s_CurrentParams[i].RedCurrentBase);
					DBG("保存通道%02d红灯电流偏差值:%03d\n",i+1,s_CurrentParams[i].RedCurrentDiff);
				}
				//set_current_params();
				udp_info.iValue[0] = 0;
			...
			{
				memcpy(&s_PhaseDescParams,&udp_info,sizeof(s_PhaseDescParams));
				udp_info.iValue[0] = 0;
				result = sendto(socketFd, &udp_info, 12, 0, (struct sockaddr*)&fromAddr, sizeof(fromAddr)); //发送消息头和消息类型
				if ( result == -1 )
        		{
			...
			{
				memcpy(&s_ChannelDescParams,&udp_info,sizeof(s_ChannelDescParams));
			...
			{
				s_ChannelDescParams.unExtraParamHead = udp_info.iHead;
				s_ChannelDescParams.unExtraParamID = udp_info.iType;
				//get_channel_desc_params();
				result = sendto(socketFd, &s_ChannelDescParams, sizeof(s_ChannelDescParams), 0, (struct sockaddr*)&fromAddr, sizeof(fromAddr)); //发送消息头和消息类型
				if ( result == -1 )
        		{
            		DBG("sendto udp info error!!!\n");
			...
			{
				memcpy(&s_PatternDescParams,&udp_info,sizeof(s_PatternDescParams));
			...
			{
				memcpy(&s_PlanDescParams,&udp_info,sizeof(s_PlanDescParams));
				udp_info.iValue[0] = 0;
				result = sendto(socketFd, &udp_info, 12, 0, (struct sockaddr*)&fromAddr, sizeof(fromAddr)); //发送消息头和消息类型
				if ( result == -1 )
        		{
			...
			{
				memcpy(&s_DateDescParams,&udp_info,sizeof(s_DateDescParams));
			...
			{
				memcpy(&S_CountDownParams,udp_info.iValue,sizeof(S_CountDownParams));
			...
			{
				memcpy(&s_ComParams,&udp_info,sizeof(s_ComParams));
