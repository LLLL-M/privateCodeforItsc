...
//考虑到校验要放到前端来处理，故此处重新开始校验    
    ...
    for(i = 0 ; i < NUM_CHANNEL; i++)
    {
        ...
        if((pSignalControlpara->stChannel[i].nControllerID != 0) && (sTemp != (i+1)))
        {
            ...
            log_error_cn("ErrorCode: ERROR_SUBSCRIPT_CHANNEL , Error Content: 通道表%d下标不符合规范. \n",i+1);
...
static int IsChannelLegal(SignalControllerPara *pSignalControlpara,unsigned short *nPhaseArray,unsigned short *nFollowPhaseArray)
{
    ...
    for(i = 0 ; i < NUM_CHANNEL ; i++)
    {
        ...
        else if ((pSignalControlpara->stChannel[i].nControllerType == FOLLOW) || (pSignalControlpara->stChannel[i].nControllerType == OTHER))//OTHER 表示行人跟随
        {
